...
class FCNHeadCAC(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        super(FCNHeadCAC, self).__init__(**kwargs)
        if num_convs == 0:
    ...
    def forward_train(self, inputs, img_metas, gt_semantic_seg, train_cfg):
        ...
        ## this is the predictions yielded by the vanilla classifier (baseline)
        pre_self_x = x.clone()  

        ## get predictions via the classifier enriched by current contextual information
        x = self.post_refine_proto_v2(x=feat, pred=x, proto=self.conv_seg.weight.squeeze())    

        ## get predictions for mimicking, and the following KL loss (actually is got by label smoothing) lets the enriched classifier behave like the adaptive perspective 
