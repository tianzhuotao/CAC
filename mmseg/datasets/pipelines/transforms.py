...
class SETR_Resize(object):
    ...
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ori_scale =  results['scale']
            if self.setr_multi_scale:
                if min(results['scale']) < self.crop_size[0]:
                    h, w = results['img'].shape[:2]
                    if h > w:
                        tmp_h, tmp_w = min(results['scale']) * h / w, min(results['scale'])
                    else:
                        tmp_h, tmp_w = min(results['scale']), min(results['scale']) * w / h  
                    results['img'], _ = mmcv.imrescale(
                        results['img'], (tmp_h, tmp_w), return_scale=True)                        
                    results['img'], _ = mmcv.imrescale(
                        results['img'], (h, w), return_scale=True)    
                ...
                new_scale = results['scale']
                # print('ori: {}, new: {}, crop_size: {}'.format(ori_scale, new_scale, self.crop_size[0]))
            img, scale_factor = mmcv.imrescale(
                results['img'], results['scale'], return_scale=True)
            # the w_scale and h_scale has minor difference
            # a real fix should be done in the mmcv.imrescale in the future
